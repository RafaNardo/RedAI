...
static async Task<IResult> GenerateIdeas(RedAIDbContext db, JobQueue jobs, Guid id) { if (!await db.Campaigns.AnyAsync(c => c.Id == id)) return Results.NotFound(); if (!await db.CampaignStrategies.AnyAsync(s => s.CampaignId == id && s.Status == "approved")) return Results.BadRequest(new { error = "Strategy must be approved before ideas." }); return await Start(db, jobs, "ideas-generation", "campaign", id, 15, async (sp, ct) => { var context = sp.GetRequiredService<RedAIDbContext>(); var ai = sp.GetRequiredService<IAIClient>(); var campaign = await context.Campaigns.FindAsync([id], ct) ?? throw new KeyNotFoundException(); var strategy = await context.CampaignStrategies.FirstOrDefaultAsync(x => x.CampaignId == id && x.Status == "approved", ct) ?? throw new InvalidOperationException("Strategy must be approved before ideas."); var ideas = ai.Mode == "mock" ? Enumerable.Range(1, 15).Select(i => new { title = $"Ideia de proteção {i:00}", pillar = i % 2 == 0 ? "Educação" : "Autoridade", contentType = i % 3 == 0 ? "Carrossel" : "Post único", description = "Abordagem educativa e humana para proteção patrimonial." }).ToArray() : (await ai.CompleteStructuredAsync(new StructuredTextRequest("ideas-generation", "Você é o IdeaGenerator do RED AI. Use exclusivamente o Brand DNA aprovado e a estratégia aprovada. Gere exatamente 15 ideias distintas. Responda estritamente no schema.", new { campaign = new { campaign.Name, campaign.Objective, campaign.Context, campaign.TargetCount }, brandDna = JsonDocument.Parse((await context.BrandProfiles.FirstAsync(b => b.ProjectId == campaign.ProjectId, ct)).ProfileJson).RootElement, strategy = JsonDocument.Parse(strategy.StrategyJson).RootElement }, "ideas", sp.GetRequiredService<IContractSchemaCatalog>().Load("ideas")), ct)).RootElement.GetProperty("ideas").EnumerateArray().Select(x => new { title = x.GetProperty("title").GetString() ?? string.Empty, pillar = x.GetProperty("pillar").GetString() ?? string.Empty, contentType = x.GetProperty("contentType").GetString() ?? string.Empty, description = x.GetProperty("description").GetString() ?? string.Empty }).ToArray(); if (ideas.Length != 15 || ideas.Select(x => x.title.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).Count() != 15 || ideas.Any(x => string.IsNullOrWhiteSpace(x.title) || string.IsNullOrWhiteSpace(x.pillar) || string.IsNullOrWhiteSpace(x.contentType) || string.IsNullOrWhiteSpace(x.description))) throw new InvalidOperationException("IdeaGenerator must return 15 unique ideas with title, pillar, content type and description."); context.ContentIdeas.RemoveRange(context.ContentIdeas.Where(i => i.CampaignId == id)); context.ContentIdeas.AddRange(ideas.Select((idea, index) => new ContentIdea { CampaignId = id, Ordinal = index + 1, Title = idea.title, Pillar = idea.pillar, ContentType = idea.contentType, Description = idea.description })); await context.SaveChangesAsync(ct); }); }
