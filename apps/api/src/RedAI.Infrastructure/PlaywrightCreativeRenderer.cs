...
""";
        return html.Replace("__BG__", Color(layout.Palette.Background)).Replace("__PRIMARY__", Color(layout.Palette.Primary)).Replace("__ACCENT__", Color(layout.Palette.Accent)).Replace("__BACKGROUND__", background.Length == 0 ? "none" : background).Replace("__SIZE__", HeadlineSize(layout.Headline.Size).ToString()).Replace("__LOGO__", LogoPosition(layout.Logo.Position)).Replace("__TEMPLATE__", Text(layout.Template)).Replace("__HEADLINE__", Highlight(layout.Headline)).Replace("__SUPPORT__", string.IsNullOrWhiteSpace(layout.SupportingText) ? string.Empty : $"<p class=\"support\">{Text(layout.SupportingText)}</p>").Replace("__CTA__", string.IsNullOrWhiteSpace(layout.Cta) ? string.Empty : $"<p class=\"cta\">{Text(layout.Cta)}</p>");
    }
